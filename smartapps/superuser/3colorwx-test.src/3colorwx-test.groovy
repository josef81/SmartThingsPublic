/**
 *  NEW TEST SUMMER2015 WXLIGHT 3 Color
 *
 *	
...
preferences
{
    section("Select Motion Detector") {
        input "motion_detector", "capability.motionSensor", title: "Where?", required:false
    }
...
def checkForWeather() {
	...
	//  SUBFREEZING BLUE	
    if (conditions.current_observation.temp_f < 20 ) {
        	color = "SFZBlue"
   ...
   //  HOT RED	
    if (conditions.current_observation.temp_f >= 90 ) {
        	color = "HotRed"
 ...
 sendcolor(color)
    ...
    httpGet("https://api.forecast.io/forecast/631cb9063a07f3ad6953487ed8ce0962/$location.latitude,$location.longitude") {response -> 
		...
		if (response.data) {
            ...
            def wxalerts = response.data.alerts.title
			log.debug "Alert: ${wxalerts}, Actual current temp: ${tempFar}, Precipitation probability now: ${precipprob}, thisHour: ${thisHour}, nextHour ${nextHour}"
    		if ((thisHour >0.15) || (nextHour >0.15)) {
	    		color = "Purple" 
                sendcolor(color)
    	    	log.debug "Greater than 15% chance of rain in this or the next hour, setting light to Purple."
    		}
            //if (wxalerts.contains("Warning")) {
            if (wxalerts !=("Null")) {
            	color = "Red"
                sendcolor(color)
                flashLights()
...
//if (wxalerts.contains("warning")) {
      log.debug "Alert: ${wxalerts}"
	//def newKeys = alerts?.collect{it.type + it.date_epoch} ?: []
	//log.debug "Severe Weather Alert: newKeys: $newKeys"

//if (newKeys.contains(alert.type + alert.date_epoch) && descriptionFilter(alert.description)) {
                //color = "Red"
               // sendcolor(color)
				//flashLights()
